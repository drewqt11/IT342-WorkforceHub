...
export default function OvertimeRequestsPage() {
  ...
  const fetchCounts = async () => {
    try {
      ...
      const [pendingRes, approvedRes, rejectedRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/requests/status/PENDING?page=0&size=1`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/requests/status/APPROVED?page=0&size=1`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/requests/status/REJECTED?page=0&size=1`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ])
      ...
      // Set default counts to 0 in case of error
      setCounts({
        total: 0,
        pending: 0,
        approved: 0,
  ...
  useEffect(() => {
    ...
    const fetchOvertimeRequests = async () => {
      ...
      const url = statusFilter === "all" 
        ? `${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/requests/status/all?page=${page}&size=${size}&sortBy=${sortBy}&direction=${direction}`
        : `${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/requests/status/${statusFilter}?page=${page}&size=${size}&sortBy=${sortBy}&direction=${direction}`
      ...
      setOvertimeRequests(data.content)
  ...
  const handleApprove = async (requestId: string) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/request/${requestId}/review?status=APPROVED`,
        {
          method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      toast.error(err instanceof Error ? err.message : "Failed to approve overtime request")
    }
  }
  ...
  const handleReject = async (requestId: string) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/hr/overtime/request/${requestId}/review?status=REJECTED`,
        {
          method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      toast.error(err instanceof Error ? err.message : "Failed to reject overtime request")
    }
  }
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                ...
                <div className="h-12 w-12 bg-[#FEF3C7] dark:bg-[#92400E]/30 rounded-full flex items-center justify-center">
                  <Clock className="h-6 w-6 text-[#D97706] dark:text-[#F59E0B]" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {loading ? (
              ...
              <div className="text-center py-12 border border-dashed border-[#E5E7EB] dark:border-[#374151] rounded-lg bg-[#F9FAFB] dark:bg-[#111827]/50">
                ...
                <p className="text-[#6B7280] dark:text-[#9CA3AF] max-w-md mx-auto mb-6">
                  We couldn't find any overtime requests matching your current filters. Try adjusting your search criteria.
                </p>
                <Button
                ...
                <Table>
                  <TableHeader className="bg-[#F9FAFB] dark:bg-[#111827]">
                    <TableRow className="hover:bg-[#F3F4F6] dark:hover:bg-[#1F2937] border-b border-[#E5E7EB] dark:border-[#374151]">
                      ...
                      <TableHead className="text-[#4B5563] dark:text-[#D1D5DB] font-medium">Start Time</TableHead>
                      <TableHead className="text-[#4B5563] dark:text-[#D1D5DB] font-medium">End Time</TableHead>
                      ...
                      >
                        ...
                        <TableCell className="text-[#4B5563] dark:text-[#D1D5DB]">
                          <Badge variant="outline" className="bg-[#e8f3fa] text-[#148ab8] border-[#99e3f6] dark:bg-[#134E4A]/30 dark:text-[#14B8A6] dark:border-[#134E4A]">
                            {format(new Date(`2000-01-01T${request.startTime}`), "hh:mm a")}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-[#4B5563] dark:text-[#D1D5DB]">
                          <Badge variant="outline" className="bg-[#e8f3fa] text-[#148ab8] border-[#99e3f6] dark:bg-[#134E4A]/30 dark:text-[#14B8A6] dark:border-[#134E4A]">
                            {format(new Date(`2000-01-01T${request.endTime}`), "hh:mm a")}
                          </Badge>
                        </TableCell>
                          ...
                          {request.status === "PENDING" && (
                            <div className="flex gap-2">
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => handleApprove(request.otRequestId)}
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => handleReject(request.otRequestId)}
