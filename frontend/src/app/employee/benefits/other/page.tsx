...
import { Gift, Shield, DollarSign, Calendar, FileText, AlertCircle, PhilippinePeso, Plus, X, CheckCircle2 } from "lucide-react"
...
export default function OtherBenefitsPage() {
  const [benefits, setBenefits] = useState<OtherBenefit[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [selectedPlan, setSelectedPlan] = useState<OtherBenefit | null>(null)
  ...
  useEffect(() => {
    const fetchOtherBenefits = async () => {
      try {
        ...
        // Fetch other benefits (excluding Health and Retirement)
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch other benefits")
        }

        const data = await response.json()
        // Filter out Health and Retirement plans
        const filteredBenefits = data.filter((benefit: OtherBenefit) => 
          !benefit.planType.toLowerCase().includes('health') && 
          !benefit.planType.toLowerCase().includes('retirement')
        )
  ...
  const handleEnrollClick = (benefit: OtherBenefit) => {
        ...
        <div className="w-full max-w-6xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-[#6B7280] dark:text-[#9CA3AF] mt-1">View your other benefit plans</p>
            </div>
          </div>
        ...
        {benefits.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-[#E5E7EB] dark:border-[#374151] rounded-lg bg-[#F9FAFB] dark:bg-[#111827]/50">
            <div className="relative w-16 h-16 mx-auto mb-4">
              ...
              <div className="absolute inset-1 bg-white dark:bg-[#1F2937] rounded-full flex items-center justify-center">
                <Gift className="h-8 w-8 text-[#6B7280] dark:text-[#9CA3AF]" />
              </div>
            </div>
            <h3 className="text-xl font-medium text-[#1F2937] dark:text-white mb-2">No Other Benefits Found</h3>
            <p className="text-[#6B7280] dark:text-[#9CA3AF] max-w-md mx-auto">
              There are no other benefit plans available at the moment.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Card className="overflow-hidden border border-[#E5E7EB] dark:border-[#374151] shadow-md hover:shadow-lg transition-all duration-200 flex flex-col h-full mt-1.5">
                  ...
                  <CardContent className="p-4 pt-0 flex-grow flex flex-col justify-between">
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        <p className="text-sm font-medium text-[#4B5563] dark:text-[#D1D5DB]">Plan Type:</p>
                        <p className="text-sm text-[#6B7280] dark:text-[#9CA3AF]">
                          {benefit.planType}
                        </p>
                      </div>
      ...
      <Dialog open={isEnrollDialogOpen} onOpenChange={setIsEnrollDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Please review the plan details before confirming your enrollment.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <div className="text-sm text-[#6B7280]">
                ...
                <p>Plan Type: {selectedPlan?.planType}</p>
